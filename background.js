...
chrome.runtime.onMessageExternal.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'execute_api') {
        ...
        try {
            ...
            // [KODE BARU] Berikan akses God Mode untuk OS Canvas Hub agar tidak dibaca "unknown"
            if (urlStr.includes('/canvas') || urlStr.includes('/desktop')) {
                appId = "canvas_hub";
            } else if (appPathSegment) {
        ...
        chrome.storage.local.get([`flowork_app_state_${appId}`, 'flowork_registry_cache'], (result) => {
            const bypassedApps = ['content-forge', 'webshield-auditor', 'seo-checker', 'tt-deepscan', 'yt-deepscan', 'canvas_hub'];
