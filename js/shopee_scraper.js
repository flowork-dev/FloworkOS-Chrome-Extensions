...
(async () => {
    try {
        ...
        // =========================================================
        // LAYER 8: AGGRESSIVE DOM & JSON BLOB FALLBACK (THE NEW PATCH)
        // INFO: Menambal kebocoran "Unknown" & "0" dari perubahan struktur Shopee terbaru
        // =========================================================
        try {
            // 1. Shop Name Fallback (DOM Text & Links)
            if (payload.shopIntel.shopName === "Unknown Shop" || payload.shopIntel.shopName === "") {
                const shopNameEls = document.querySelectorAll('.page-product__shop .shop-name, [class*="shop-name"], [class*="ShopName"], a[href^="/shop/"]');
                for (let el of shopNameEls) {
                    let text = (el.innerText || "").trim();
                    if (text.length > 2 && text.length < 50 && !text.toLowerCase().includes('kunjungi') && !text.toLowerCase().includes('produk')) {
                        payload.shopIntel.shopName = text;
                        break;
                    }
                }
            ...
            // 2. Followers Fallback (DOM Text)
            if (payload.shopIntel.followerCount === 0) {
                const followerEls = document.querySelectorAll('div, span');
                for (let el of followerEls) {
                    let text = (el.innerText || "").trim().toLowerCase();
                    if (text.includes('pengikut') || text.includes('follower')) {
                        let match = text.match(/([0-9.,]+)\s*(rb|k|jt|m)?\s*(?:pengikut|follower)/i);
                        if (match) {
                            let num = parseFloat(match[1].replace(/,/g, '.'));
                            let mult = (match[2] || "").toLowerCase();
                            if (mult === 'rb' || mult === 'k') num *= 1000;
                            if (mult === 'jt' || mult === 'm') num *= 1000000;
                            payload.shopIntel.followerCount = Math.floor(num);
                            break;
                        }
                    }
                }
            ...
            // 3. Stock Fallback (DOM Text)
            if (payload.inventoryXRay.totalRealStock === 0) {
                const stockEls = document.querySelectorAll('div, span, label');
                for (let el of stockEls) {
                    let text = (el.innerText || "").trim().toLowerCase();
                    if (text.includes('tersisa') || text.includes('sisa') || text.includes('stok') || text.includes('stock')) {
                        let match = text.match(/(?:tersisa|sisa|stok|stock)\s*([0-9.,]+)\s*(?:buah|pcs)?/i);
                        if (match) {
                            payload.inventoryXRay.totalRealStock = parseInt(match[1].replace(/[.,]/g, ''));
                            break;
                        }
                    }
                }
            ...
            // 4. Update the Default Varian if stock was found via fallback
            if (payload.inventoryXRay.totalRealStock > 0 && payload.inventoryXRay.models.length > 0) {
                if (payload.inventoryXRay.models[0].realStock === 0) {
                    payload.inventoryXRay.models[0].realStock = payload.inventoryXRay.totalRealStock;
                }
            }

            // 5. Update Live Session Shop Name if it was Unknown
            if (payload.liveStreamIntel.activeSessions.length > 0) {
                payload.liveStreamIntel.activeSessions.forEach(session => {
                    if (session.hostName === "Unknown Shop" || session.hostName === "") {
                        session.hostName = payload.shopIntel.shopName !== "Unknown Shop" ? payload.shopIntel.shopName : "Shopee Live Session";
                    }
                });
            }

            // 6. Deep JSON extraction dari tag <script type="application/json"> yang sering disembunyikan Shopee
            const jsonScripts = document.querySelectorAll('script[type="application/json"]');
            jsonScripts.forEach(script => {
                try {
                    let strData = script.textContent;

                    if (payload.shopIntel.shopName === "Unknown Shop") {
                        let m = strData.match(/"shopName":"([^"]+)"/i) || strData.match(/"name":"([^"]+)"/i);
                        if (m && m[1] && !m[1].includes('.jpg')) payload.shopIntel.shopName = m[1];
                    }
                    if (payload.inventoryXRay.totalRealStock === 0) {
                        let m = strData.match(/"stock":(\d+)/i) || strData.match(/"normal_stock":(\d+)/i);
                        if (m && m[1]) payload.inventoryXRay.totalRealStock = parseInt(m[1]);
                    }
                    if (payload.shopIntel.followerCount === 0) {
                        let m = strData.match(/"follower_count":(\d+)/i);
                        if (m && m[1]) payload.shopIntel.followerCount = parseInt(m[1]);
                    }
                } catch(e){}
            });

        } catch(e) {
            payload.errorLog.push("Layer 8 Fallback Error: " + e.message);
        }
